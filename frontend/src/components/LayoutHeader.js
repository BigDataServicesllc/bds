...
const LayoutHeader = ({ currentPage, onNavigate, onToggleLanguage, language }) => {
  ...
  // 👉 En estas rutas queremos SIEMPRE header sólido, aunque no haya scroll
  const needsSolidHeader =
    location.pathname === "/terminos" ||
    location.pathname === "/privacidad";

  // Estado final que usamos para colores / fondo
  ...
  const NavButton = ({ item }) => {
    ...
    const getColorClasses = () => {
      // Header transparente (solo cuando NO es sólido)
      if (!isSolidHeader) {
        return isActive
          ? "text-white font-semibold"
          : "text-white hover:text-[#06E8D1]";
      }
      // Header sólido (blanco)
      return isActive
  ...
  return (
    ...
    >
      ...
      {isMenuOpen && (
        <div className={`md:hidden border-t ${isSolidHeader ? 'border-gray-200 bg-white' : 'border-white/10 bg-primary-dark/95'}`}>
          <nav className="px-4 py-3 flex flex-col space-y-2">
            ...
            <button
              ...
              className={`
                mt-2 inline-flex items-center justify-center px-3 py-1.5 rounded-md border text-sm
                ${isSolidHeader
                  ? 'text-gray-900 border-gray-400 hover:bg-gray-200'
                  : 'text-white border-white/40 hover:bg-white/10'
                }
